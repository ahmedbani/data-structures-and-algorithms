...
function transformToLis(obj){
  // Solution code here...
  let newArr =[];
  newArr.push(`<li>name: ${obj.name}</li>`,`<li>age: ${obj.age}</li>`);
  return newArr;
  // why it keeps giving me undefined and fails even though it works logically and on an online compiler 
}
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    for (var j = 0; j < input[i].length; j++) {
      if(typeof(input[i][j])==='number'){
        if(input[i][j] % 5 === 0){
          temp.push(Math.pow(2,input[i][j]));
        }
      }
    }
    newArr.push(temp);
...
let findMaleAndFemale = (data) => {
  ...
  names = names.substr(names.indexOf(" ") + 1);
  return names;
};
