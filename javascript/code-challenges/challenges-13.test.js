...
const longestString = (arr) => {
  ...
  if (arr.length === 0) {
    ...
    let longest = arr.reduce((acc, val, idx) => {
      if (temp < val.length) {
        temp = val.length;
        acc = idx;
      }
...
const findHappiness = (arr) => {
  ...
  let temp = arr.reduce((acc, val) => {
    if (val.indexOf(":)") > -1) happiness.push(val);
    return acc;
  }, "");
  return happiness;
};
...
const allHappy = (arr) => {
  ...
  let happiness = [];
  let temp = arr.reduce((acc, val) => {
    if (val.indexOf(":)") > -1) happiness.push(true);
    return acc;
  }, "");
  if(happiness.length === arr.length)
    return true;
  else 
    return false;
};
